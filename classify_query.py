...
def validate(records):
    ...
    testdata = get_training_data(True)

    y_pred = []
    y_test = [testdata[intent].name for intent in sorted(testdata.keys())]

    for obs in sorted(testdata.keys()):   
        vector = model.tfidf.transform([obs]).toarray()[0]
        y_pred.append(model.model.predict(vector))

    y_pred = [obs[0] for obs in y_pred]
